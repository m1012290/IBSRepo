...
public class UserDetailsDaoImpl extends BaseDBDAO implements UserDetailsDao {
    ...
    public BaseVO getUserDetails(BaseVO baseVO) {
        if (Utils.isEmpty(baseVO)) {
            throw new BusinessException("userdetails.nullinput", null,
                "Input recieved for user details [" + baseVO + "] is null");
        }

        UserVO userVO = (UserVO) baseVO;
        if (Utils.isEmpty(userVO.getLoginName()) && Utils.isEmpty(userVO.getUserId())) {
            throw new BusinessException("userdetails.nullinput", null,
                "Input recieved for user details [" + baseVO + "] is null");
        }
        List<IbsUser> ibsUserList = null;
        if (!Utils.isEmpty(userVO.getUserId())) {
            ibsUserList = queryUserDetailsByUserId(userVO);
        } else if (!Utils.isEmpty(userVO.getLoginName())) {
            ibsUserList = queryUserDetailsByLoginName(userVO);
        }

        if (Utils.isEmpty(ibsUserList)) {
            throw new BusinessException("userdetails.invalidinput", null,
                "Invalid user details recieved, please check the same");
        }
        try {
    ...
    private List<IbsUser> queryUserDetailsByLoginName(UserVO userVO) {
        // getHibernateTemplate().findByNamedParam("from IbsMasLookup ibsMasLookUp where ibsMasLookUp.id.category=:identifier",
        // "identifier", categoryName);
        return getHibernateTemplate().findByNamedParam(
            "from IbsUser ibsUser where ibsUser.loginName=:loginName", "loginName",
            userVO.getLoginName());
    }

    @SuppressWarnings("unchecked")
    private List<IbsUser> queryUserDetailsByUserId(UserVO userVO) {
        return getHibernateTemplate().findByNamedParam("from IbsUser ibsUser where ibsUser.id=:id",
            "id", userVO.getUserId());
    }
    ...
    private void populateUserDetails(List<IbsUser> ibsUserList, UserVO userVO) {
        ...
        userVO.setUserName(ibsUserList.get(0).getDescription());
    ...
    private void populateUserContact(IbsUser ibsUser, UserVO userVO) {
        ...
        addressVO.setLattitude(Utils.isEmpty(ibsUser.getIbsContact().getLocationLattitude()) ? null
            : String.valueOf(ibsUser.getIbsContact().getLocationLattitude()));
        addressVO.setLongitude(Utils.isEmpty(ibsUser.getIbsContact().getLocationLongitude()) ? null
            : String.valueOf(ibsUser.getIbsContact().getLocationLongitude()));
    ...
    private void populateUserRolePrivilege(IbsRoles roles,
            List<UserRolePrivilege> userRolePrivileges) {
        UserRolePrivilege userRolePrivilege = null;
        ProductVO productVO = null;
        Iterator<IbsRolesProductPrivileges> rolesProductPrivilege =
            roles.getIbsRolesProductPrivilegeses().iterator();
        while (rolesProductPrivilege.hasNext()) {
            ...
            userRolePrivilege
                    .setGenerateClosingSlip(roleProductPrivileges.getGenerateClosingslip());
