...
public class QuotationMB extends BaseManagedBean implements java.io.Serializable{
	...
	try{
		QuoteSlipMB quoteSlipMB = (QuoteSlipMB) FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get("quoteSlipMB");
		...
		for(QuoteDetailVO quoteDetailVO : this.quoteDetailList ){
			...
			companyVO.setCode(quoteDetailVO.getCompanyCode());
			...
			quoteDetVO.getQuoteSlipPrmDetails().setCoverDescription(quoteDetailVO.getQuoteSlipPrmDetails().getCoverDescription());
		...
		policyDetails=(PolicyVO) ServiceTaskExecutor.INSTANCE.executeSvc("quotationSvc","createQuotation",this.policyDetails);
		
	}catch(Exception ex){
		FacesContext.getCurrentInstance().addMessage("ERROR_QUOTATION_SAVE", new FacesMessage(FacesMessage.SEVERITY_ERROR,null, "Error saving quote details, please try again after sometime"));
	    ex.printStackTrace();
	}
	FacesContext.getCurrentInstance().addMessage("MESSAGE_SUCCESS", new FacesMessage(FacesMessage.SEVERITY_INFO,"Quote Details Captured "," successfully"));
		return null;
	}
